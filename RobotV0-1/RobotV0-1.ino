...
#include <SerialNetwork.h>   // needed for RF434 network
...
float coeffGlissementRotation = 1.17;
int i = 0;
...
void loop() {
  int getSerial = Raspberry.Have_message();
  ...
  if (PendingReqSerial != 0x00)
  {
    ...
    Raspberry.DataToSendSerial(); // bidouille pour forcer la gateway serial udp a lire le cote udp
  }
...
void MoveToward(int orientation, int length) {
  ...
  int s1;
  float* nbPasToDo = MoveTowardCalculation(orientation, length) ;

  for (int i = 0; i <= 3; i++)
  {
    Serial.println(nbPasToDo[i]);
  }
  if (orientation != 0) {
    s1 = (orientation < 0) ? -1 : +1;
  }
  float nbPasCorrige = abs(nbPasToDo[0]) * coeffGlissementRotation;
...
float*  MoveTowardCalculation(int orientation, int length) {
  ...
  float *nbPasToDo = (float *) malloc(sizeof(float) * 4);
  int nbPasG1;
  int nbPasD2;
  int nbPasG2;
  Serial.print("sinus:");
  ...
  nbPasToDo[0] = (((rayonRobot * PI / 180) * orientation) / (2 * PI * rayonRoue)) * nbPas;
  nbPasToDo[1] = (rayonRobot * sin(angleRadian) / (2 * PI * rayonRoue)) * nbPas;
  //  nbPasToDo[1]=rayonRobot*sin(orientation)*nbPas/(2*PI*rayonRoue);
  //  nbPasG1=nbPasD1[0];
  nbPasToDo[2] = (length / (2 * PI * rayonRoue)) * nbPas;
  nbPasToDo[3] = (length / (2 * PI * rayonRoue)) * nbPas;
  //  nbPasG2=-nbPasD2;
  Serial.print("pas D1:");
  Serial.println(nbPasToDo[0]);
  Serial.print("pas D2:");
  Serial.println(nbPasToDo[2]);
  return (nbPasToDo);
