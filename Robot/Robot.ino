...
void loop() {
  ...
  if (reboot == false && appStat != 0xff)         //  wait reboot complete to act
  {
    ...
    { // Update robot position
      ...
      if ((millis() - timeMotorStarted) > 500 && bitRead(diagMotor, diagMotorPbSynchro) == false)   // wait for motors to run enough
      {
...
void PingFrontBack()
{
  SendScanResultSerial (PingFront(), PingBack());
  timePingFB = millis();
}
...
void SendScanResultSerial (int distF, int distB)   // send scan echo data to the server
{
  ...
  PendingDataReqSerial[8] = 0x41; //"A" angle
